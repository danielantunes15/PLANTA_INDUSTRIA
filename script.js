...
// Dados dos Setores
// DICA: Você provavelmente precisará ajustar 'pos: {x, z}' para bater com o desenho
const SETORES = [
...
function init() {
    ...
    // Nevoeiro leve, mas mais distante para não apagar o mapa
    scene.fog = new THREE.FogExp2(0x0a0f1d, 0.005);

    // 2. Câmera
    camera = new THREE.PerspectiveCamera(50, container.clientWidth / container.clientHeight, 0.1, 1000);
    camera.position.set(0, 80, 0); // Câmera olhando de cima para facilitar ajuste
...
function createEnvironment() {
    ...
    // Se sua imagem não for quadrada, ajuste o (100, 100) abaixo
    const planeGeo = new THREE.PlaneGeometry(100, 100);

    const planeMat = new THREE.MeshStandardMaterial({ 
        map: floorTexture,
        color: 0xffffff, // Branco para mostrar cores reais
        roughness: 0.9,
        metalness: 0.1
    });
    ...
    floor.position.y = -0.1; // Chão levemente recuado
    floor.receiveShadow = true;
    scene.add(floor);

    // Grid removido para limpar a visão da planta
    // const grid = new THREE.GridHelper(100, 50, 0x1e293b, 0x1e293b);
    // scene.add(grid);

    // === ILUMINAÇÃO REFORÇADA ===
    // Luz ambiente mais forte para ler o desenho
...
function renderCables() {
    ...
    if(adm && port) drawCable(adm.pos, port.pos);
}
...
function drawCable(p1, p2) {
    ...
    points.push(new THREE.Vector3(midX, 2, midZ));
...
function animate() {
    ...
    if (intersects.length > 0) {
        if (INTERSECTED != intersects[0].object) {
            ...
            INTERSECTED.material.color.setHex(0x2dd4bf);
...
function repairSystem() {
    ...
    interactables.forEach(m => m.userData.originalColor = 0xffffff);
